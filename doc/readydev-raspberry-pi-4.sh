...
_EOF_
...
# ignore le gros fichier dans git
...
-> creer un dossier {tmp}
-> ignorer le dossier {tmp} dans le fichier {.gitignore}
-> déplacer le gros fichier dans le dossier {tmp}
...
git add .
git commit -m "Initial commit"
git push
...
# clone le depot distant git (rdvdoc-02)
...
git clone https://github.com/gkesse/rdvdoc.git rdvdoc-02
...
# copie le depot local git (rdvdoc-03)
...
cp -r ./rdvdoc/ ./rdvdoc-03
...
# determine la taille du dossier (.git) dans (rdvdoc-03)
...
du -sh ./rdvdoc-03/.git
...
<< _EOF_
...
_EOF_
...
# supprime le dossier (.git) dans (rdvdoc-03)
...
rm -rf ./rdvdoc-03/.git
...
# fusionne les dossiers git (rdvdoc-03) dans (rdvdoc-02)
...
-> fermer vscode {si terminal ouvert et utilise}
-> fusionner les repertoires git
-> cocher -> remplacer tous les fichiers du repertoire
-> ouvrir vscode
...
# pousse les modifications dans le dossier (rdvdoc-02)
...
git add .
git commit -m "Initial commit"
git push
